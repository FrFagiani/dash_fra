...
def filter_dataframe(df: pd.DataFrame, dfcolsel) -> pd.DataFrame:
    ...
    with modification_container:
        to_filter_columns = st.multiselect("Filtra il dataframe:", dfcolsel)  # df.columns
...
indices = 'data/shp/Indices_output.csv'
...
# dfex_col = df.loc[:, ~df.columns.isin(['geometry', 'Cont_ID'])]
...
nofiltcolmap = df_filtcol+(['geometry', 'Cont_ID', 'DPSVI', 'GDPnum', 'POPnum'])
